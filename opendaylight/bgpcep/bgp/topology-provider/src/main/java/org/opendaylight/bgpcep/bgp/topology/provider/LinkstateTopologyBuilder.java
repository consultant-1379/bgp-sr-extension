...
import org.opendaylight.yang.gen.v1.urn.opendaylight.params.xml.ns.yang.bgp.linkstate.rev150210.linkstate.attribute.UnidirLinkDelay;
...
import org.opendaylight.yang.gen.v1.urn.tbd.params.xml.ns.yang.network.sr.poc.topology.rev141112.Ted1Builder;
import org.opendaylight.yang.gen.v1.urn.tbd.params.xml.ns.yang.network.sr.poc.topology.rev141112.Uint24;
import org.opendaylight.yang.gen.v1.urn.tbd.params.xml.ns.yang.network.sr.poc.topology.rev141112.UnidirectionalLinkdelay.Flags;
import org.opendaylight.yang.gen.v1.urn.tbd.params.xml.ns.yang.network.sr.poc.topology.rev141112.network.topology.topology.link.igp.link.attributes.isis.link.attributes.ted.AdjSidBuilder;
import org.opendaylight.yang.gen.v1.urn.tbd.params.xml.ns.yang.network.sr.poc.topology.rev141112.network.topology.topology.link.igp.link.attributes.isis.link.attributes.ted.UnidirectionalLinkdelayBuilder;
import org.opendaylight.yang.gen.v1.urn.tbd.params.xml.ns.yang.network.sr.poc.topology.rev141112.network.topology.topology.node.igp.node.attributes.prefix.PrefixSidBuilder;
...
public class LinkstateTopologyBuilder extends AbstractTopologyBuilder<LinkstateRoute> {
    ...
    // Ericsson SR
    private final Map<String, MplsLabel> nodeSrgbBase = new HashMap<>();
    ...
    private static org.opendaylight.yang.gen.v1.urn.tbd.params.xml.ns.yang.network.isis.topology.rev131021.IgpLinkAttributes1 isisLinkAttributes(
        ...
        if (la != null) {
            ...
            // Ericsson BGP-SR begin
            Ted1Builder l1b = null;
            if ((la.getSrAdjIds() != null) && (!la.getSrAdjIds().isEmpty())) {
                if (l1b == null)
                    l1b = new Ted1Builder();
                AdjSidBuilder adjSidBuilder = new AdjSidBuilder();
                if ((la.getSrAdjIds().get(0).getFlags() != null)
                        && (la.getSrAdjIds().get(0).getFlags() instanceof IsisAdjFlagsCase)) {
                    final IsisAdjFlagsCase f = (IsisAdjFlagsCase) la.getSrAdjIds().get(0).getFlags();
                    // FF NOTE: rev141112.AdjSid.Flags constructor is
                    // Flags(java.lang.Boolean _b, java.lang.Boolean _f)
                    adjSidBuilder.setFlags(
                            new org.opendaylight.yang.gen.v1.urn.tbd.params.xml.ns.yang.network.sr.poc.topology.rev141112.AdjSid.Flags(
                                    f.isBackup(), f.isAddressFamily()));
                }
                if (la.getSrAdjIds().get(0).getWeight() != null) {
                    adjSidBuilder.setWeight(la.getSrAdjIds().get(0).getWeight().getValue());
                }
                if (la.getSrAdjIds().get(0).getSidLabelIndex() != null) {
                    final SidLabelIndex sli = la.getSrAdjIds().get(0).getSidLabelIndex();
                    if (sli instanceof LocalLabelCase) {
                        adjSidBuilder.setSid(((LocalLabelCase) sli).getLocalLabel().getValue());
                    } else if (sli instanceof SidCase) {
                        adjSidBuilder.setSid(((SidCase) sli).getSid());
                    }
                }
                l1b.setAdjSid(adjSidBuilder.build());
            }

            if (la.getUnidirLinkDelay() != null) {
                UnidirLinkDelay bgpUld = la.getUnidirLinkDelay();
                UnidirectionalLinkdelayBuilder linkStateUld = new UnidirectionalLinkdelayBuilder();

                if (l1b == null)
                    l1b = new Ted1Builder();
                if (bgpUld.getFlags() != null) {
                    linkStateUld.setFlags(new Flags(bgpUld.getFlags().isA()));
                }
                if (bgpUld.getDelay() != null) {
                    linkStateUld.setDelay(new Uint24(bgpUld.getDelay().getValue()));
                }
                l1b.setUnidirectionalLinkdelay(linkStateUld.build());
            }

            if (l1b != null)
                tb.addAugmentation(
                        org.opendaylight.yang.gen.v1.urn.tbd.params.xml.ns.yang.network.sr.poc.topology.rev141112.Ted1.class,
                        l1b.build());
            // Ericsson BGP-SR end
        }
    ...
    private void createNode(final WriteTransaction trans, final UriBuilder base,
        ...
        LOG.info("LinkStateTopologyBuilder.createNode: nodeId {}", nid);

        if (attr != null) {
            LOG.info("LinkStateTopologyBuilder.createNode: step #1");
            final LinkStateAttribute linkStateAttribute = attr.getLinkStateAttribute();
            if (linkStateAttribute != null) {
                LOG.info("LinkStateTopologyBuilder.createNode: step #2");
                final MplsLabel localLabel = parseLocalLabelFromNodeAttributes(linkStateAttribute);
                if (localLabel != null) {
                    LOG.info("LinkStateTopologyBuilder.createNode: step #3 {} {}",
                            nid.getValue(), localLabel.getValue());
                    nodeSrgbBase.put(nid.getValue(), localLabel);
                }
            }
        }
        // Ericsson BGP-SR end
    ...
    // Ericsson BGP-SR start
    private MplsLabel parseLocalLabelFromNodeAttributes(final LinkStateAttribute linkStateAttribute) {
        MplsLabel localLabel = null;

        if (linkStateAttribute instanceof NodeAttributesCase) {
            LOG.info("LinkStateTopologyBuilder.parseLocalLabelFromNodeAttributes: step #1");
            final NodeAttributes na = ((NodeAttributesCase)linkStateAttribute).getNodeAttributes();
            if (na == null) {
                LOG.info("LinkStateTopologyBuilder.parseLocalLabelFromNodeAttributes: exit na null");
                return null;
            }
            final SrCapabilities srCapabilities = na.getSrCapabilities();
            if (srCapabilities != null && srCapabilities.getSidLabelIndex() != null) {
                final SidLabelIndex sidLabelIndex = srCapabilities.getSidLabelIndex();
                LOG.info("LinkStateTopologyBuilder.parseLocalLabelFromNodeAttributes: step #2");
                if (sidLabelIndex != null && sidLabelIndex instanceof LocalLabelCase) {
                    LOG.info("LinkStateTopologyBuilder.parseLocalLabelFromNodeAttributes: step #3");
                    localLabel = ((LocalLabelCase)sidLabelIndex).getLocalLabel();
                }
            }
        }
    ...
    private void createPrefix(final WriteTransaction trans, final UriBuilder base,
        ...
        if ((pa != null) && (pa.getSrPrefix() != null)) {
            PrefixSidBuilder prefSidBuilder = new PrefixSidBuilder();
            if ((pa.getSrPrefix().getFlags() != null) && (pa.getSrPrefix().getFlags() instanceof IsisPrefixFlagsCase)) {
                final IsisPrefixFlagsCase f = (IsisPrefixFlagsCase) pa.getSrPrefix().getFlags();
                prefSidBuilder.setFlags(
                        new org.opendaylight.yang.gen.v1.urn.tbd.params.xml.ns.yang.network.sr.poc.topology.rev141112.PrefixSid.Flags(
                                f.isExplicitNull(), f.isNodeSid(), f.isNoPhp(), f.isReadvertisement()));
            }
            if (pa.getSrPrefix().getAlgorithm() != null) {
                prefSidBuilder.setAlgorithm((short) pa.getSrPrefix().getAlgorithm().getIntValue());
            }
            if (pa.getSrPrefix().getSidLabelIndex() != null) {
                final SidLabelIndex sli = pa.getSrPrefix().getSidLabelIndex();
                if (sli instanceof LocalLabelCase) {
                    prefSidBuilder.setSid(((LocalLabelCase) sli).getLocalLabel().getValue());
                } else if (sli instanceof SidCase) {
                    prefSidBuilder.setSid(((SidCase) sli).getSid());
                }
            }

            LOG.info("LinkStateTopologyBuilder.createPrefix: nodeId {}", node.getValue());
            final MplsLabel localLabel = nodeSrgbBase.get(node.getValue());
            if (localLabel != null) {
                LOG.info("LinkStateTopologyBuilder.createPrefix: step #1 {}", localLabel.getValue());
                prefSidBuilder.setLocalLabel(localLabel.getValue());
                nodeSrgbBase.remove(node.getValue());
            }

            org.opendaylight.yang.gen.v1.urn.tbd.params.xml.ns.yang.network.sr.poc.topology.rev141112
                    .Prefix1Builder srPrefix1Builder = new org.opendaylight.yang.gen.v1.urn.tbd.params.xml.ns.yang
                    .network.sr.poc.topology.rev141112.Prefix1Builder();
            srPrefix1Builder.setPrefixSid(prefSidBuilder.build());
            pb.addAugmentation(
                    org.opendaylight.yang.gen.v1.urn.tbd.params.xml.ns.yang.network.sr.poc.topology.rev141112.Prefix1.class,
                    srPrefix1Builder.build());
        }
        // Ericsson BGP-SR end
