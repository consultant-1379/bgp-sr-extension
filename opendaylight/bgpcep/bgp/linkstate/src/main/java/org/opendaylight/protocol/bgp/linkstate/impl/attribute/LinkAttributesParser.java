...
import org.opendaylight.yang.gen.v1.urn.opendaylight.params.xml.ns.yang.bgp.linkstate.rev150210.Uint24;
import org.opendaylight.yang.gen.v1.urn.opendaylight.params.xml.ns.yang.bgp.linkstate.rev150210.UnidirectionalLinkDelay.Flags;
...
import org.opendaylight.yang.gen.v1.urn.opendaylight.params.xml.ns.yang.bgp.linkstate.rev150210.linkstate.attribute.UnidirLinkDelayBuilder;
...
public final class LinkAttributesParser {
    ...
    private static final int UNIDIR_LINK_DELAY = 1104;
    // Ericsson BGP-SR end
    ...
    static LinkStateAttribute parseLinkAttributes(final Multimap<Integer, ByteBuf> attributes, final ProtocolId protocolId) {
        ...
        for (final Entry<Integer, ByteBuf> entry : attributes.entries()) {
            ...
            case PEER_SET_SID_CODE:
                ...
                break;
            // Ericsson BGP-SR begin
            case UNIDIR_LINK_DELAY:
                parseUnidirLinkDelay(value, builder);
                break;
            // Ericsson BGP-SR end
            default:
    ...
    // Ericsson BGP-SR begin
    private static void parseUnidirLinkDelay(final ByteBuf value, final LinkAttributesBuilder builder) {
        final UnidirLinkDelayBuilder unidirLinkDelayBuilder = new UnidirLinkDelayBuilder();
        final ByteBuf v = value.readSlice(4);
        unidirLinkDelayBuilder.setFlags(new Flags(new Boolean(v.getBoolean(0))));
        unidirLinkDelayBuilder.setDelay(new Uint24(new Long(v.getMedium(1))));
        builder.setUnidirLinkDelay(unidirLinkDelayBuilder.build());
        LOG.debug("Parsed Unidirectional Link Delay {}", builder.getUnidirLinkDelay());
    }
    // Ericsson BGP-SR end
    ...
    static void serializeLinkAttributes(final LinkAttributesCase linkAttributesCase, final ByteBuf output) {
        ...
        // Ericsson BGP-SR begin
        ifPresentApply(linkAttributes.getUnidirLinkDelay(), value -> TlvUtil.writeTLV(UNIDIR_LINK_DELAY,  LinkAttributesParser.serializeUnidirLinkDelay((UnidirLinkDelay)value), output));
        // Ericsson BGP-SR end
    ...
    // Ericsson BGP-SR begin
    public static ByteBuf serializeUnidirLinkDelay(final UnidirLinkDelay linkDelay) {
        final ByteBuf value = Unpooled.buffer();
        final BitArray flag = new BitArray(8);
        flag.set(0, linkDelay.getFlags().isA());
        flag.toByteBuf(value);
        value.writeBytes(Unpooled.copyMedium(linkDelay.getDelay().getValue().intValue()));
        return value;
    }
    // Ericsson BGP-SR end
