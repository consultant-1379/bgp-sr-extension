...
// Ericsson BGP-SR extension start
...
public final class NodeAttributesParser {
    ...
    // Fake Segment-Routing capabilities RIB data to be used only for reproducing
    // RIB segment routing capabilities entries by adding to case
    // TlvUtil.LOCAL_IPV4_ROUTER_ID for example what follows:
	//     builder.setSrCapabilities(createFakeSrCapabilities());
    static SrCapabilities createFakeSrCapabilities() {
        return new SrCapabilitiesBuilder().setMplsIpv4(Boolean.TRUE)
                                          .setMplsIpv6(Boolean.TRUE).setSrIpv6(Boolean.FALSE)
                                          .setSidLabelIndex(new LocalLabelCaseBuilder()
                                          .setLocalLabel(new MplsLabel(220000L)).build())
